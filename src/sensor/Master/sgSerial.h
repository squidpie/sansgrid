...
#define NUM_BYTES 98			// Number of bytes sent or received over SPI per packet
...
// Opens serial device for reading/writing, configures ports, sets order data bits 
// are shifted in as MSB or LSB, and sets the clock frequency. Function is called 
// prior to sending or receiving any data over the serial line. 
...
// Transmit number of bytes "size" from SansgridSerial structure to Slave over SPI. 
// For full duplex a second SansgridSerial is passed in with a second size integer 
// to receive data from the slave in return. The most significant bit first in 
// Big Endian byte order. Number of bytes is not to exceed control byte plus 
// IP address destination/source bytes plus payload bytes. 
int8_t sgSerialSend(SansgridSerial *tx, SansgridSerial **rx, uint32_t size);
// Receive number of bytes "size" and places in SansgridSerial structure. The most 
// significant bit first in Big Endian byte order. Number of bytes is not to exceed 
// control byte plus IP address destination/source bytes plus payload bytes. 
int8_t sgSerialReceive(SansgridSerial **rx, uint32_t size);
// Function is called when all SPI input and output is completed. Stops SPI from being 
// transmitted and received. 
int8_t sgSerialClose(void);

#endif // __SG_SERIAL_H__
